...
// Inicializamos el array de errores.
	...
	{ // Aqui dentro realizaremos la actualización de los datos y validaciones.
		...
		if (count($errores) == 0)
		{
			// Comprobamos si hay cambio de contraseña.
			if ($_POST['password'] != $_POST['oldpassword'])
			...
			$sql = sprintf("update users set name='%s', surname='%s',password='%s',birthday='%s' where nickname='%s'",mysql_real_escape_string($_POST['name']), mysql_real_escape_string($_POST['surname']), $password, cambiaf_mysql($_POST['birthday']), $_SESSION['nickname']);
	...
	if (count($errores) != 0 || empty($_POST)) // Mostramos el formulario.
	{
		...
		else echo $fila['password']; ?>"/>
					<input type="hidden" name="oldpassword" id="oldpassword" size="10" maxlength="15" value="<?php echo $fila['password']; ?>"/>
				</li>
				<li>
					<label for="email">E-mail address:</label>
					<input type="email" name="email" id="email" placeholder="test@info.local" size="20" maxlength="50" disabled value="<?php echo $fila['email']; ?>"/>
				</li>
				<li>
					<label for="birthday">Birthday:</label>
					<input type="date" name="birthday" id="birthday" value="<?php if (!empty($_POST['birthday'])) echo $_POST['birthday'];
		else echo $fila['birthday']; ?>"/>
				</li>
				<li>
	...
	echo "ERROR: Incorrect access to this page.<br/><br/>You must be logged in.";
?>